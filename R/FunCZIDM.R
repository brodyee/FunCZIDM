...
#' @seealso
#'   \code{\link{calcRA}},
#'   \code{\link{calcDeltaRA}},
#'   \code{\link{calcAlphaDiv}},
#'   \code{\link{calcDeltaAlphaDiv}},
#'   \code{\link[=FunCZIDM-package]{FunCZIDM-package}}
#' @examples
#' \dontrun{
#' # toy example with included data
#' library(FunCZIDM)
#'
#' data(infantData)
...
#' @title Runs the Shiny App for Visualization
#' @description Runs the Shiny app for visualizing the output of 
#' \code{FunCZIDM}.
#' @param useCondaEnv (logical, default=FALSE) Whether to use the conda
#' environment for running the app. If TRUE, the conda environment named
#' "shinyPy" will be used. If FALSE, the python from reticulate will be used.
#' @param ... Additional arguments to pass to \code{shinyAppDir}.
#' @export
runPlotApp <- function(useCondaEnv = FALSE, ...) {
  app_dir <- system.file("app", package = "FunCZIDM")  
  if (app_dir == "")
    stop("App directory not found. Try re-installing `FunCZIDM`.")

  shiny::shinyOptions(useCondaEnv = isTRUE(useCondaEnv))
