...
Gammaproteobacteria, and the sum of all other relative abundance's. We then plot
the distribution of these three on the simplex at three different time point. 
...
the multiplicative change from these relative abundance. Using the same 
...
smallest average mean $\beta_{jp}(t)$ over time and smallest $|\beta_{jp}(t)|$.
Most others are close to zero, so when the gestional age is changed the 
concentation parameter of Gammaproteobacteria is decreased the most. This causes 
...
data, then checking the coverage of the true beta functions is below.
...
samples can be uploaded locally. [This](toDo) video give a walk through on how 
