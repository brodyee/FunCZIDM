...
Concurrent Zero-Inflated Dirichlet-Multinomial Regression Model with Application
 to Infant Microbiome" to provide examples
of how you would use and gain inference from the model. This package contains 
the data from the case study and the samples from the case study used in the 
paper, along with the scripts used to run the model and simulations. More 
details on reproducing the results in the paper can be found in the 
...
we convert the categorical variables to factors and ensure the reference level 
matches that used in the case study of the main manuscript.
...
Now we can double check the data are set up correctly. Note the counts data must
be a matrix, and the covariates data frame should only contain the covariates. 
...
```{r}
is.matrix(infantCounts)

colnames(infantCounts)
```

The `infantCounts` data contains the counts of the class level using 16S rRNA 
genes from infant stool samples.
...
and how much to thin by (`thin`). For demonstration purposes, we only do 85 
iterations with a 75 burn-in while thinning to every 10th iteration. To assess 
convergence, you can return the burn-in samples by setting `returnBurnIn=TRUE`. 
...
If there are covariates that are assumed to not have a time-varying effect, one 
can specify the indices of these covariates in the `covWithoutVC` argument. In 
the case study, we assume the period of study to be time-invariant. By default,
all covariates have a time-varying effect.
...
or "output.rds" by default. This is useful when running multiple chains in order
to combine them after all chains have finished running. One can also save a csv
...
```{r}
combinedOutput <- readRDS(system.file("extdata", "plotCaseStudySamples.rds",
                          package = "FunCZIDM"))
...
$(\alpha(t))$, the multiplicative difference in relative abundance 
$(\Delta_v \text{RA}_{jp}(t))$ and $\alpha$-diversity functions 
$(\Delta_v \alpha_p(t))$, and the functional coefficients $(\beta_{jp}(t))$.

We begin by looking at the proportion of the credible interval of
$\beta_{jp}(t)$ that contains the null function. By identifying the lowest 
proportions, we can narrow down which covariates and categories to investigate 
further, allowing for easier exploration of the $\alpha$-diversity and relative 
abundance.
...
multiplicative differences credible intervals do not contain the null function. 
Unless there is interest in particular covariates, the top variables are a good 
starting point for exploration.

## Relative Abundance

The relative abundance and multiplicative difference in relative abundance can be
...
time. We choose the 77th index, which is around day 25 of life.
...
With `ggtern`, we can view the distribution on the simplex. To plot it on a 
2-simplex, we must collapse some categories to have 3 categories total. Below we
have the simplex of Bacilli, Gammaproteobacteria, and the sum of all other 
relative abundances. We then plot the distribution of these three on the simplex
at three different time points. This gives us an idea of how the distribution 
shifts over time.

```{r, eval=hasggtern}
# Getting the data at 3 different times and combining
S10 <- as.data.frame(t(RA[10,,])) # RA samples around day 4
# sum of each sample's RA other than Bacilli and Gammaproteobacteria
S10["Other"] <- 1 - rowSums(S10[c("Bacilli", "Gammaproteobacteria")]) 
S77 <- as.data.frame(t(RA[77,,])) # RA samples around day 25
S77["Other"] <- 1 - rowSums(S77[c("Bacilli", "Gammaproteobacteria")])
S150 <- as.data.frame(t(RA[150,,])) # RA samples around day 47
S150["Other"] <- 1 - rowSums(S150[c("Bacilli", "Gammaproteobacteria")])
# getting the group labels
...
ggtern(S, aes(Other, Bacilli, Gammaproteobacteria, color = group)) +
  stat_density_tern(
...
the multiplicative difference from these relative abundances. Using the same 
...
plot(x, deltaRAMeans[, taxa[1]], ylim=ylim, type="l", 
     ylab="Mean Multiplicative Difference in Relative Abundance",
     xlab="Day of Life")
...
Additionally, we can find the distribution of the multiplicative difference at a
particular time.

```{r}
print(paste("Time point", rownames(deltaRA[[1]])[77]))
par(mfrow=c(3, 1))
hist(deltaRA[[1]][77, taxa[1], ], main=taxa[1], 
     xlab="Multiplicative Difference in Relative Abundance", 
     xlim=ylim, breaks=20)
hist(deltaRA[[1]][77, taxa[2], ], main=taxa[2], 
     xlab="Multiplicative Difference in Relative Abundance", 
     xlim=ylim, breaks=20)
hist(deltaRA[[1]][77, taxa[3], ], main=taxa[3], 
     xlab="Multiplicative Difference in Relative Abundance", 
     xlim=ylim, breaks=20)
```

One may want to see what proportion of the multiplicative difference credible 
interval contains the null effect. This can be done using the samples to 
estimate the credible interval. By leaving `forCats` NULL, we get all the 
categories multiplicative difference.
...
oneInCI <- (lowerCI <= 1)*(upperCI >= 1) # Null effect is 1
colMeans(oneInCI)
```
...
plot the mean multiplicative difference over a range of changes. 
...
S27["Other"] <- 1 - rowSums(S27[c("Clostridia", "Gammaproteobacteria")])
covProfile <- c(1, 30, 0, 0, 0, 0, 0, 0, .3)
RA <- calcRA(combinedOutput, covProfile=covProfile)
S30 <- as.data.frame(t(RA[85,,]))
S30["Other"] <- 1 - rowSums(S30[c("Clostridia", "Gammaproteobacteria")])
covProfile <- c(1, 33, 0, 0, 0, 0, 0, 0, .3)
RA <- calcRA(combinedOutput, covProfile=covProfile)
S33 <- as.data.frame(t(RA[85,,]))
S33["Other"] <- 1 - rowSums(S33[c("Clostridia", "Gammaproteobacteria")])
S27$group <- paste("GA of", 27)
S30$group <- paste("GA of", 30)
S33$group <- paste("GA of", 33)
cols <- c("Clostridia", "Gammaproteobacteria", "Other", "group")
S <- rbind(S27[,cols], S30[,cols], S33[,cols])

ggtern(S, aes(Other, Clostridia, Gammaproteobacteria, color = group)) +
  stat_density_tern(
    aes(fill = after_stat(level)),
    geom = "path", bins = 50, alpha = 0.5
  ) + 
...
The functions to get the $\alpha$-diversity functions 
$(\alpha(t))$, the multiplicative difference in $\alpha$-diversity functions 
$(\Delta_v \alpha_p(t))$ work similarly to the relative abundance functions. 
Using the same covariate profile, we can find the diversity over time using 
`calcAlphaDiv`. We have one extra parameter to set than `calcRA`, `l`, 
that controls the weight given toward the smaller categories. 
...
plot(x, meanDiv, ylim=ylim, type="l", 
     ylab=paste0("Mean Alpha-Diversity (l=", l, ")"),
     xlab="Day of Life")
```

We can calculate the multiplicative difference from this $\alpha$-diversity function
...
contain the null effect. The inputs are the same as `calcDeltaRA` with 
...
for (cov in 1:8) {
  ...
  oneInCI <- (CI[1,] <= 1)*(CI[2,] >= 1) # Null effect is 1
...
plot(x, meanDivFunc, type="l", col=cols[1], ylim=c(.9, 1.5),
     ylab="Mean Multiplicative Difference in Alpha-Diversity",
     xlab="Day of Life")
for (cov in 2:8) {
...
We can plot the simplex for a few different covariate profiles to see the 
...
At times, the functional covariate itself may be of interest. These are 
particularly useful for seeing which covariate is causing the estimated changes.
...
largest negative average mean $\beta_{jp}(t)$ over time and the largest 
$|\beta_{jp}(t)|$. Most other coefficients are close to zero, so when the 
gestational age is changed the concentration parameter of Gammaproteobacteria is
changed the most. Since the coefficient is negative, this causes the 
distribution to get pushed away from Gammaproteobacteria's corner of the 
simplex, and raises all other relative abundance consequently.
...
The intercept functions can be found in `generatedData$betaIntercepts`. An 
example of fitting the model with the generated data, then checking the coverage
of the true beta functions are below.
...
We can plot the estimated mean and true $\beta_{jp}(t)$ below to assess 
estimation performance.

```{r}
plot(testpoints, rowMeans(betaEst), col="black", type="l", ylim=c(-1,1),
     ylab=expression(beta[jp](t)), xlab="Time")
lines(testpoints, betaFunc, col="red")
```

# `shiny` App

For visualization, the `shinyApp` associated with the package can be used
